...
export class Service {
  ...
  constructor() {
    if (
      ...
      throw new Error(
        "Appwrite configuration is missing required values. Check conf.js."
      );
    }
  ...
  async createPost({
    ...
    if (!title || !slug || !content || !userId || !userName) {
      console.error(
        "Appwrite service :: createPost :: Missing required fields"
      );
      return null;
    }
  ...
  async uploadFile(file, userId = null) {
    ...
    try {
      const permissions = userId
        ...
        : [Permission.read(Role.any())]; // Default to public read
